...
9. **Interpolation.** `Linear`: straight segments, constant speed. `Smooth`: cosine-eased timing per segment **plus** per-point `tension` (-1…+1) bowing the segment as a quadratic Bézier whose control point is offset perpendicular to the chord, scaled by tension (0 = straight, per PRD). The chord and control point are derived in **point-index order** (not traversal order), so the path is a fixed shape in the XY plane — Reverse/Ping-Pong traversal must not mirror the curve (review finding, Phase 6). Output clamped to [-1, +1]. In `Linear` mode tension is ignored.
